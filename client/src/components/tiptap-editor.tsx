...
const LINE_HEIGHTS = [
  ...
  { label: "1.15", v: "1.15" },
  { label: "1.25", v: "1.25" },
  { label: "1.5 (padrão ABNT)", v: "1.5" },
  { label: "Duplo (2.0)", v: "2" },
  { label: "2.5", v: "2.5" },
  { label: "Triplo (3.0)", v: "3" },
];

const COLORS = [
  "#000000","#434343","#666666","#999999","#b7b7b7","#cccccc","#d9d9d9","#ffffff",
  "#ff0000","#ff9900","#ffff00","#00ff00","#00ffff","#4a86e8","#0000ff","#9900ff",
  "#c00000","#e69138","#f1c232","#6aa84f","#45818e","#3c78d8","#1155cc","#674ea7",
];

const HIGHLIGHT_COLORS = [
  "#ffff00","#00ff00","#00ffff","#ff00ff","#ff9900",
  "#ffd966","#b6d7a8","#9fc5e8","#ea9999","#d9ead3",
];
...
export default function TiptapEditor({ initialData, onChange, onReady }: TiptapEditorProps) {
  ...
  useEffect(() => {
    if (editor && onReady) {
      onReady({ getData: () => editor.getHTML(), setData: (html: string) => editor.commands.setContent(html, false), _tiptap: editor });
    }
  }, [editor, onReady]);
  ...
  const toolbar = (
    <div className="word-toolbar bg-[#f3f3f3] border-b border-gray-300 select-none">
      {/* ── Barra mobile: apenas os botões essenciais ── */}
      <div className="toolbar-mobile flex items-center gap-1 px-2 py-1 border-b border-gray-200 md:hidden">
        <button
          type="button"
          title="Negrito"
          onClick={() => editor.chain().focus().toggleBold().run()}
          className={`flex items-center justify-center w-8 h-8 rounded text-sm font-bold transition-all ${
            editor.isActive('bold') ? 'bg-[#cce5ff] border border-[#4a86e8]' : 'hover:bg-[#e8e8e8]'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center w-8 h-8 rounded text-sm transition-all ${
            editor.isActive('italic') ? 'bg-[#cce5ff] border border-[#4a86e8]' : 'hover:bg-[#e8e8e8]'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center w-8 h-8 rounded text-sm transition-all ${
            editor.isActive({ textAlign: 'center' }) ? 'bg-[#cce5ff] border border-[#4a86e8]' : 'hover:bg-[#e8e8e8]'
          }`}
        >
          <AlignCenter className="w-4 h-4" />
        </button>
      </div>
      {/* ── Barra completa: todos os controles ── */}
  ...
  return (
    <div className="word-editor-wrap flex flex-col bg-[#f3f3f3]" style={{ minHeight: 600 }} data-testid="tiptap-editor">
      ...
      <style>{`
        ...
        .word-editor-content { outline: none; min-height: 24cm; }
        .word-editor-content p { margin: 0 0 0 0; text-align: justify; text-indent: 0; line-height: 1.5; min-height: 1.5em; }
        .word-editor-content h1 { font-size: 16pt; font-weight: bold; text-align: center; margin: 12pt 0 6pt; line-height: 1.3; }
        .word-editor-content h2 { font-size: 14pt; font-weight: bold; text-align: justify; margin: 10pt 0 4pt; line-height: 1.3; }
        .word-editor-content h3 { font-size: 13pt; font-weight: bold; margin: 8pt 0 4pt; line-height: 1.3; }
        .word-editor-content h4 { font-size: 12pt; font-weight: bold; margin: 6pt 0 4pt; line-height: 1.5; }
        .word-editor-content h5, .word-editor-content h6 { font-size: 12pt; font-weight: bold; margin: 4pt 0; }
        .word-editor-content ul, .word-editor-content ol { padding-left: 1.5em; margin: 4pt 0; }
        .word-editor-content li { margin: 2pt 0; line-height: 1.5; }
        .word-editor-content blockquote { margin: 8pt 0 8pt 4cm; padding: 0; font-size: 10pt; line-height: 1.3; border-left: none; font-style: normal; }
